    ...
    return (
        <div className={cn(" bg-white px-1 min-h-screen relative", isCollapsed ? 'w-20' : 'w-80')}>
            ...
            <div className="absolute -right-11 top-[30%] z-10">
                <Button onClick={() => setIsCollapsed(!isCollapsed)} variant={'outline'} className="border-none shadow-none h-20 hover:bg-slate-100">
                    {isCollapsed ? <FiChevronLeft className="rotate-180" /> : <FiChevronLeft />}
                </Button>
            </div>
        </div>
    );
