            ...
            <CardContent className="grid md:grid-cols-12 gap-5 w-full p-5">
                <div className="col-span-3 rounded-lg">
                    <div className="flex flex-col gap-1">
                        ...
                        <Label className="text-base text-primary">University of Cambridge - Fall 2023</Label>
                        <InputWithButton placeholder="Your Database Name" buttonText="Add" icon={<BsDatabase size={25} />} btnType="secondary" className="h-16 w-full focus:ring-0 focus:border-gray-100" />
                        {datasetsList.map(item => (
                            <div key={item.id} className="flex p-2 rounded-lg items-center gap-2 border hover:bg-slate-50">
                                <BsDatabase size={iconSize} />
                                <div className="flex flex-col">
                                    <Label variant={'subtitle'}>{item.title}</Label>
                                    <Label variant={'input'} >{item.description}</Label>
                                    <Label variant={'input'} className="text-xs">{item.date}</Label>
                                </div>
                                <div className="ml-auto">
                                    <Dialog>
                                        <DialogTrigger asChild>
                                            <BsGear size={20} className="text-gray-400 hover:text-primary cursor-pointer" />
                                        </DialogTrigger>
                                                ...
                                                <section className="">
                                                    <div className="space-y-2 mb-6">
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            <Checkbox /> <Label variant={'input'}>Include decimals in dataset</Label>
                                                        </div>
                                                    </div>
